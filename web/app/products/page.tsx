  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-8">
      ...
      <form className="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-3">
        ...
        />
        <select name="sort" defaultValue={searchParams.sort ?? "price"} className="rounded-md border px-3 py-2 text-sm">
        ...
        <div className="flex gap-2">
          ...
          <button type="submit" className="rounded-md bg-black px-4 py-2 text-white">Filtrar</button>
        </div>
      </form>
      ...
      <section className="mt-10">
        <div className="flex items-center justify-between">
          ...
          <p className="text-sm text-neutral-500">Página {meta.page} de {meta.totalPages} – {meta.total} productos</p>
        </div>
        ...
        <div className="mt-6 flex items-center justify-center gap-2">
          <a
            ...
            href={`?${new URLSearchParams({ ...Object.fromEntries(Object.entries(searchParams as Record<string, string | undefined>).filter(([, v]) => v !== undefined)), page: String(Math.max(1, meta.page - 1)) })}`}
          >
            Anterior
          </a>
          <a
            className="rounded-md border px-3 py-2 text-sm"
            href={`?${new URLSearchParams({ ...Object.fromEntries(Object.entries(searchParams as Record<string, string | undefined>).filter(([, v]) => v !== undefined)), page: String(Math.min(meta.totalPages, meta.page + 1)) })}`}
          >
            Siguiente
          </a>
        </div>
